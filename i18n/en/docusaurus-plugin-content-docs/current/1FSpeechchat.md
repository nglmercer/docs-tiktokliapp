...
---
# Speechchat TTS 
### Reads incoming messages, has multi-language support.
this speech bot has been created with Steamelements api.
![Streamelements API](img2/streamlabsapi.png)
### additional options
It is designed to have audio filter controls and a whiteList, you can:
- control the audio(volume up and down or mute it).
![Audio Player](img2/Reproductor.png)
- ... if you click on more player options you can speed up the audio as well as download it if it is *available*.
- add filters: each word you want to filter is separated by each space and if it is very short and has space use /a a/ in this case it will count as one word.
- Whitelist each user that if you don't read or you want to read always ---> this ignores spam filters and so on
:::tip [recommend].
if you don't want to listen type in the filter 
```
a b c d e f g h i j k l m n ñ o p q r s t u v w x y z 
``` 
or simply mute the player.
:::
#### tiene muchos filtros opciones adicionales 
## *Available Voices:*
```
var VOICE_LIST = {
